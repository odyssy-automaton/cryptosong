...
module.exports.updateSong = songData => {
  ...
  return new Promise((resolve, reject) => {
    ...
    deleteTagsForSong(Number.parseInt(songData.number, 10), task).then(task => {
      if (songData.tags.length) {
        TagModel.addOrInsertTags(
          songData.tags,
          Number.parseInt(songData.number, 10),
          task
        ).then(task => {
          ...
          task
            ...
            .then(success => {
              console.log("saved?");
              Song.findById(songData._id).then(song => {
  ...
  return new Promise((resolve, reject) => {
    ...
    song.save((err, savedSong) => {
      ...
      if (tagArray.length) {
        TagModel.addOrInsertTags(
          tagArray,
          Number.parseInt(savedSong.number, 10),
          task
        ).then(task => {
          // task.update("Song", {_id: songData._id}, songData);
          task
            ...
            .then(success => {
              console.log("saved");
              Song.findById(savedSong._id).then(song => {
                console.log(song.title);
                resolve(song);
              });
            })
...
module.exports.updateTagsOnSongs = (newTagData, task) => {
  return new Promise((resolve, reject) => {
    task.update(
      "Song",
      { "tags._id": newTagData._id },
      {
        $set: {
          "tags.$.name": newTagData.name,
          "tags.$.image": newTagData.image
        }
      }
    );
    resolve(task);
  ...
  return new Promise((resolve, reject) => {
    (function recurse(array) {
      ...
      if (!array.length) {
        ...
        let tag = array.shift();
        Song.find({ "tags._id": tag }).then(results => {
...
module.exports.getSongsByTagNames = tags => {
  return new Promise((resolve, reject) => {
    Song.find({ tagNames: { $in: tags } }).then(results => {
      resolve(results);
    });
  });
};
